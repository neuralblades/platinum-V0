...
export function usePropertyData(propertyId: string): UsePropertyDataReturn {
  const {
    ...
    queryFn: async () => {
      ...
      if (!response.success || !response.property) throw new Error('Property not found');
      if (!response.property.isOffplan) throw new Error('This is not an offplan property');
  ...
  const {
    ...
    queryFn: async () => {
      ...
      if (!response.success || !response.developer) return null;
      return response.developer as Developer;
    },
    enabled: !!developerId,
  });
