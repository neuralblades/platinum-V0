...
const PropertyMapView = ({ properties, onPropertySelect, selectedProperty }: PropertyMapViewProps) => {
  ...
  const createInfoWindowContent = useCallback((property: Property) => {
    ...
    imageSection.style.backgroundImage = `url('${getFullImageUrl(property.mainImage)}')`;
    ...
    if (property.isOffplan && property.bedroomRange) {
      bedroomsSpan.textContent = `${property.bedroomRange} Beds`;
    } else {
    ...
    if (property.isOffplan) {
      middleSpan.textContent = `Handover ${property.yearBuilt || 'TBA'}`;
    } else {
      // Format bathrooms
    ...
    if (property.isOffplan) {
      ...
      paymentSpan.textContent = `${property.paymentPlan || '60/40'} Plan`;
    ...
    button.href = property.isOffplan ? `/properties/offplan/${property.id}` : `/properties/${property.id}`;
